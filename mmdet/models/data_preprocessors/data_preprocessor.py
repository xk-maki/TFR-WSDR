...
from mmengine.model import BaseDataPreprocessor, ImgDataPreprocessor, IQDataPreprocessor, ComplexDataPreprocessor
...
@MODELS.register_module()
class IQDetDataPreprocessor(IQDataPreprocessor):
    """Data preprocessor for detection tasks with 2-channel inputs."""
...
@MODELS.register_module()
class ComplexDetDataPreprocessor(ComplexDataPreprocessor):
    """Data preprocessor for detection tasks with single-channel complex inputs (shape: (1, H, W))."""

    def __init__(self,
                 mean: Sequence[Number] = None,
                 std: Sequence[Number] = None,
                 pad_size_divisor: int = 1,
                 pad_value: Union[float, complex] = 0,  # 支持复数填充值（如 0+0j）
                 pad_mask: bool = False,
                 mask_pad_value: int = 0,
                 pad_seg: bool = False,
                 seg_pad_value: int = 255,
        ...
        # 调整均值/标准差为单通道（复数实部+虚部统一处理或分开处理）
        # 若 mean/std 为复数，需拆分为实部和虚部；若为实数，视为对实部和虚部统一归一化
        if mean is not None:
            assert len(mean) == 1, f"For single-channel complex input, mean must have 1 value, got {len(mean)}"
        if std is not None:
            assert len(std) == 1, f"For single-channel complex input, std must have 1 value, got {len(std)}"
        ...
        self.batch_augments = nn.ModuleList([MODELS.build(aug) for aug in batch_augments]) if batch_augments else None
        self.pad_mask = pad_mask
        self.mask_pad_value = mask_pad_value
        self.pad_seg = pad_seg
        self.seg_pad_value = seg_pad_value
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        """Process complex data with single channel (1, H, W)."""
        # 获取填充形状（基于单通道复数输入的尺寸）
        batch_pad_shape = self._get_pad_shape(data)

        # 调用父类预处理（处理复数输入的归一化和填充）
        data = super().forward(data=data, training=training)
        inputs, data_samples = data['inputs'], data['data_samples']

        if data_samples is not None:
            # 更新元信息：输入形状为 (1, H_padded, W_padded)
            batch_input_shape = tuple(inputs[0].shape[-2:])  # (H_padded, W_padded)
            for data_sample, pad_shape in zip(data_samples, batch_pad_shape):
                data_sample.set_metainfo({
                    'batch_input_shape': batch_input_shape,
                    'pad_shape': pad_shape
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_pad_shape = []
            for ori_input in _batch_inputs:
                assert ori_input.dim() == 3 and ori_input.shape[0] == 1, \
                    f"Single-channel complex input must be (1, H, W), got {ori_input.shape}"
                h = ori_input.shape[1]
                w = ori_input.shape[2]
            ...
            assert _batch_inputs.dim() == 4 and _batch_inputs.shape[1] == 1, \
                f"Batch input must be (N, 1, H, W), got {_batch_inputs.shape}"
            h = _batch_inputs.shape[2]
            w = _batch_inputs.shape[3]
            pad_h = int(np.ceil(h / self.pad_size_divisor)) * self.pad_size_divisor
            pad_w = int(np.ceil(w / self.pad_size_divisor)) * self.pad_size_divisor
            batch_pad_shape = [(pad_h, pad_w)] * _batch_inputs.shape[0]
        else:
            raise TypeError(f"Unsupported input type: {type(_batch_inputs)}")
        return batch_pad_shape

    # 保持mask和语义分割的填充逻辑不变（假设mask和分割图为实数）
